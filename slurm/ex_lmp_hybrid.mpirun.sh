...
#SBATCH --time=01:05:00              # Wall time limit (hh:mm:ss)
...
export PPN=$(($SLURM_NTASKS / $SLURM_JOB_NUM_NODES))
echo "\$PPN=$PPN"
for i in `seq 1 5`;
do
mpirun -n $SLURM_NTASKS -ppn $PPN $(which lmp) -in in.lmp.lj -pk omp $SLURM_CPUS_PER_TASK -sf omp
done
